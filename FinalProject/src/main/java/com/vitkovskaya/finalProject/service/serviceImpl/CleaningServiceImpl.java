...
public class CleaningServiceImpl implements CleaningService {
    ...
    public boolean addCleaning(Map<String, String> map, long id) throws ServiceException {
        ...
        logger.log(Level.DEBUG, (map.get(ConstantName.PARAMETER_CLEANING_PRICE)));
        ...
        cleaning.setCleanerId(id); // FIXME: 08.02.2020 из атрибутов взять id cleaner
    ...
    public Optional<Cleaning> findCleaningById(long cleaningId) throws ServiceException {
        CleaningDaoImpl cleaningDao = new CleaningDaoImpl();
        EntityTransaction transaction = new EntityTransaction();
        Optional<Cleaning> cleaningOptional = Optional.empty();
        transaction.beginNoTransaction(cleaningDao);
        try {
    ...
    @Override
    public Optional<BigDecimal> getCleaningPrice(long cleaningId) throws ServiceException {
        CleaningDaoImpl cleaningDao = new CleaningDaoImpl();
        EntityTransaction transaction = new EntityTransaction();
        Optional<Cleaning> cleaningOptional;
        Optional<BigDecimal> priceOptional = Optional.empty();
        Cleaning cleaning;
        transaction.begin(cleaningDao);
        try {
            cleaningOptional = cleaningDao.findById(cleaningId);
            cleaning = cleaningOptional.orElseThrow(() -> new ServiceException("Cleaning not found!"));
            priceOptional = Optional.of(cleaning.getPrice());
        } catch (DaoException e) {
            transaction.rollback();
            logger.log(Level.ERROR, "Exception while committing transaction", e);
            throw new ServiceException(e);
